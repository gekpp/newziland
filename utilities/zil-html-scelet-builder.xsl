...
<xsl:template name="write-styles">
  <link rel="stylesheet" href="{$workspace}/css/common.css" type="text/css" media="screen, projection" />
  <link rel="stylesheet" href="{$workspace}/css/page_with_galery.css" type="text/css" media="screen, projection" />
  <xsl:choose>
    <xsl:when test="('home'=$root-page) or number(/data/contact-page-by-1st-menu/entry/@id)">
      <link rel="stylesheet" href="{$workspace}/css/like_main_page.css" type="text/css" media="screen, projection" />
    </xsl:when>
  </xsl:choose>
</xsl:template>
	...
	<div class="middle" id="header">
			...
			<div class="content">
				<div class="center_content">
					<div class="left_column">
						<div class="header_1st_line" style="float: right"><xsl:value-of select="/data/ml-strings/entry[name='развитие территории']/value"/></div>
						<div class="header_2nd_line" style="float: right; padding-left: 45px;"><xsl:value-of select="/data/ml-strings/entry[name='полуостров зил']/value"/></div>
		...
		<div class="sidebar sideLeft menu">
		  <xsl:variable name="item-class">
			...
			<div ><a href="{$root}/{$url-language}/home"><span class="{$item-class}"><xsl:value-of select="/data/ml-strings/entry[name='main']/value"/></span></a></div>
                      </xsl:when>
                      <xsl:otherwise>
			<div ><a href="{$root}/home"><span class="{$item-class}"><xsl:value-of select="/data/ml-strings/entry[name='main']/value"/></span></a></div>
                      </xsl:otherwise>
                    </xsl:choose>
